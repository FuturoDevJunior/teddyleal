...
describe('UserService', () => {
  ...
  beforeEach(() => {
    userRepository = {
      ...
      findByEmail: jest.fn(async (email) => email === 'user@email.com' ? { id: '1', name: 'User', email, passwordHash: 'hashed-password', createdAt: new Date(), updatedAt: new Date(), deletedAt: null } : null),
      findById: jest.fn(),
    };
  ...
  it('deve registrar um novo usuário com e-mail sanitizado', async () => {
    const user = await service.register({ name: 'User', email: '  USER@email.com ', password: 'password' });
