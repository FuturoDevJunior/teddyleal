...
export interface IShortUrlRepository {
  ...
  create(data: { originalUrl: string; shortCode: string; userId?: string | null }): Promise<ShortUrl>;
...
export class ShortUrlRepository implements IShortUrlRepository {
  ...
  async create(data: { originalUrl: string; shortCode: string; userId?: string | null }): Promise<ShortUrl> {
